...
export function Button({
  ...
  const variantClasses = {
    ...
    secondary:
      'bg-transparent border-2 border-accent-ice text-accent-ice hover:bg-accent-ice hover:text-surface-dark',
  ...
  return (
    <button
      className={`inline-flex items-center justify-center rounded-lg font-medium transition-all duration-200 ${variantClasses[variant]} ${sizeClasses[size]} ${fullWidth ? 'w-full' : ''} ${disabledClasses}`}
      disabled={disabled}
      {...props}
    >
      {children}
